...
### 2.2 La Realidad de las Emergencias Médicas en España

#### 2.2.1 Estructura del Sistema de Emergencias Extrahospitalarias

España cuenta con **17 comunidades autónomas** con sus propios servicios de emergencias médicas (SEM), coordinados a través del número único europeo **112** disponible desde 1997. Los principales operadores son:

- **SUMMA 112** (Madrid): Servicio de Urgencia Médica de Madrid.
- **SAMUR** (Ayuntamiento de Madrid): Servicio de Asistencia Municipal de Urgencia y Rescate.
- **SEM** (Cataluña), **SAMU** (Valencia), **061** (Andalucía, Galicia, Baleares).

El sistema es **dirigido por médicos** (*physician-led*), con presencia de facultativos en las unidades de Soporte Vital Avanzado (SVA). Desde 2007, existe un estándar nacional para los Técnicos en Emergencias Sanitarias (TES), con formación de ciclo formativo de grado superior de 2 años. ([Wikipedia EMS Spain](https://en.wikipedia.org/wiki/Emergency_medical_services_in_Spain))

El objetivo de tiempo de respuesta del SAMU español es **inferior a 15 minutos**. Sin embargo, las cifras reales varían significativamente según la densidad urbana, la distribución geográfica y la hora del día. En zonas rurales, los tiempos de respuesta pueden superar los 20-30 minutos.

#### 2.2.2 Estado Actual de la Infraestructura

| Componente | Estado Actual | Vulnerabilidades |
|---|---|---|
| Comunicaciones | Radio VHF + 4G limitado | Sin cifrado extremo a extremo; saturación en incidentes masivos; sin redundancia automática |
| Navegación | GPS comercial | Vulnerable a spoofing de señal; sin integración con semáforos inteligentes |
| Telemetría | OBD-II básico | Sin análisis predictivo; datos en silos no integrados con el centro de mando |
| Coordinación | Centros 112 fragmentados por CCAA | Interoperabilidad limitada entre regiones; sin visibilidad de flota unificada |
| Datos médicos | Formularios en papel o sistemas propietarios | Sin transmisión anticipada al hospital receptor; sin acceso a historial |
| Seguridad | Autenticación básica usuario/contraseña | Sin Zero-Trust; auditoría limitada; sin detección de anomalías de comportamiento |

**Puntos de dolor críticos documentados:**

1. **Saturación de redes en crisis**: los incidentes masivos colapsan las redes 4G/5G, aislando a las ambulancias del centro de coordinación exactamente cuando más se necesita comunicación.
2. **Ausencia de visibilidad en tiempo real**: el centro de mando no conoce el estado mecánico, el nivel de combustible ni las constantes vitales del paciente hasta que el técnico lo comunica verbalmente.
3. **Ruteo estático e ineficiente**: la navegación no considera atascos dinámicos, el estado de semáforos ni las restricciones de tráfico en tiempo real.
4. **Mantenimiento reactivo costoso**: las averías se detectan cuando ya han ocurrido, inmovilizando el vehículo en servicio activo.
5. **Silos de información irreconciliables**: los datos médicos, logísticos y mecánicos residen en sistemas separados sin posibilidad de correlación automática.
...
### 2.7 Referentes Internacionales: Benchmark de Sistemas de Emergencia

El diseño del Gemelo Digital se apoya en el análisis comparativo de los sistemas EMS más avanzados del mundo, extrayendo las mejores prácticas y adaptándolas al contexto español.

#### 2.7.1 Reino Unido — London Ambulance Service (LAS)

El **London Ambulance Service** es el servicio de ambulancias urbano con mayor volumen del mundo, con más de **2.000 llamadas de emergencia al día** y una flota de más de 450 ambulancias ([LAS Annual Report](https://www.londonambulance.nhs.uk/)). Sus métricas de referencia son:

- **Tiempo objetivo para Categoría 1** (emergencias inmediatas con riesgo vital): media ≤ 7 minutos, con al menos el **90 % de casos en ≤ 15 minutos**.
- **Sistema CAD** (Computer-Aided Dispatch): el LAS utiliza el sistema **Cleric/Terrafix** para despacho asistido por ordenador con posicionamiento en tiempo real de toda la flota.
- **Proyecto iBOLS** (Integrated Benefits Optimization and Load System): análisis predictivo para optimizar la posición proactiva de ambulancias antes de que ocurran los incidentes, basado en patrones históricos y eventos programados (partidos de fútbol, conciertos, clima).

El LAS lleva años explorando la telemetría avanzada de vehículos integrada con el CAD. Su experiencia demuestra que la **predicción de ubicación óptima** puede reducir el tiempo de respuesta medio en entornos urbanos densos en hasta un **15-20 %** sin añadir recursos adicionales.

#### 2.7.2 Países Bajos — RAVU (Regionale Ambulancevoorziening Utrecht)

Los Países Bajos implementaron a nivel nacional el sistema **"Dynamische standplaatsen"** (posicionamiento dinámico de ambulancias), un precursor directo del concepto de gemelo digital operativo. El sistema:

- Recalcula continuamente las **posiciones óptimas de espera** de cada ambulancia en función de la demanda histórica, tráfico en tiempo real y disponibilidad de recursos hospitalarios.
- Ha demostrado una **reducción del 8-12 % en tiempos de respuesta** en zonas urbanas sin aumentar el número de vehículos.
- Integra datos de los **RAV** (Regionale Ambulancevoorziening) con los sistemas de información hospitalaria para coordinar la recepción anticipada de pacientes.

#### 2.7.3 Estados Unidos — King County EMS (Seattle/Washington)

El **King County EMS** de Seattle/Washington es reconocido globalmente como el sistema EMS con la **tasa de supervivencia por PCR extrahospitalaria más alta del mundo**, con resultados que superan el **60 % para fibrilación ventricular presenciada** (frente al promedio mundial de 5-10 %). Los factores clave según estudios publicados:

- **Respuesta en menos de 4 minutos**: objetivo estricto de tiempo de respuesta que ha demostrado ser el determinante más importante de supervivencia.
- **Integración total con desfibriladores automáticos externos (AED)** en espacios públicos: el gemelo digital puede integrar la localización de AEDs cercanos al incidente.
- **Sistema de telemedicina**: médicos de urgencias en el hospital guían al personal de ambulancia mediante vídeo en tiempo real durante maniobras críticas.
- **Datos en tiempo real**: el médico supervisor tiene acceso a ECG, SpO2 y signos vitales del paciente desde el hospital antes de su llegada.

Nuestro Gemelo Digital implementa capacidades equivalentes a las del King County EMS, pero sobre infraestructura estándar y escalable mediante HPE GreenLake.

#### 2.7.4 Tabla Comparativa Internacional

| País / Servicio | Tiempo respuesta objetivo | Tasa supervivencia PCR | Tecnología diferencial |
|---|---|---|---|
| **España (SAMU/112)** | < 15 min (objetivo nacional) | ~8-12 % (estimado) | Radio VHF + 4G; sin telemetría avanzada |
| **UK (London Ambulance)** | Cat.1: media 7 min, 90 % en 15 min | ~12-15 % (estimado) | CAD con posicionamiento, iBOLS predictivo |
| **Países Bajos (RAVU)** | < 15 min (objetivo nacional) | ~16-20 % (estimado) | Posicionamiento dinámico, integración RAV |
| **EE.UU. (King County)** | < 4 min (Seattle urbano) | > 60 % (FV presenciada) | Telemetría 12 derivaciones, telemedicina vídeo |
| **Objetivo con Gemelo Digital** | < 8 min (urbano) | > 18 % (proyección) | Gemelo + PdM + IA + federated learning |

*Fuente: [Global Resuscitation Alliance](https://www.globalresuscitationalliance.org/), [ERC Guidelines 2021](https://www.cprguidelines.eu/), informes anuales de cada servicio.*

#### 2.7.5 Lecciones Aprendidas de los Referentes

| Lección | Referente | Aplicación en Gemelo Digital |
|---|---|---|
| El posicionamiento proactivo supera al despacho reactivo | LAS / Países Bajos | Módulo de optimización de posición de espera (idle positioning) |
| La telemedicina en tránsito mejora el pronóstico del paciente | King County | Módulo de telemedicina con vídeo sobre 5G (Sección 4.8) |
| La integración con AEDs ciudadanos reduce la mortalidad | King County | API de localización de DEA del Registro Nacional de Desfibriladores |
| Los datos de ECG anticipados al hospital mejoran el triaje | King County | Transmisión FHIR de ECG 12 derivaciones en tiempo real |
| El entrenamiento continuo del personal es tan importante como la tecnología | LAS | Plan de formación por competencias con simulación (Sección 6.8) |
...
**HPE Edgeline EL8000 por ambulancia** — servidor diseñado para entornos hostiles ([HPE Edgeline Store](https://buy.hpe.com/us/en/compute/edgeline-systems/c/1008663980)):

| Parámetro | Especificación | Justificación |
|---|---|---|
| Procesador | Intel Xeon D-2146NT (8 cores, 2,0 GHz) | Inferencia IA local en tiempo real; eficiencia energética |
| Memoria | 64 GB DDR4 ECC | Buffer de telemetría y modelos IA residentes en memoria |
| Almacenamiento | 2× 1 TB NVMe SSD RAID 1 | Redundancia ante fallos de disco; velocidad para series temporales |
| Conectividad | 4× 10GbE + WiFi 6E + 5G + Satélite | Multi-RAT para conectividad redundante y sin interrupciones |
| Seguridad hardware | TPM 2.0 (NIST SP 800-147) | Arranque seguro verificado, almacenamiento de claves criptográficas |
| Consumo | 45 W máximo | Compatible con sistemas eléctricos de ambulancia (12/24 V) |
| Temperatura operativa | −20 °C a 60 °C | Condiciones extremas dentro del vehículo |
| Certificaciones | IP67, MIL-STD-810G | Vibración, humedad, polvo y golpes en entornos hostiles |

**Gateway OBD-II y sensores vehiculares:**

| Componente | Modelo | Parámetros monitorizados |
|---|---|---|
| Gateway OBD-II | OBDLink MX+ (CAN bus) | RPM, temperatura motor, presión aceite, velocidad, nivel combustible (>200 PIDs) |
| GPS/IMU | U-blox ZED-F9P (RTK) | Posición centimétrica, velocidad, rumbo, detección spoofing, 10 Hz |
| Acelerómetro/Giróscopo | Bosch BMI088 | Vibración, impactos, detección de accidente, 400 Hz |
| Temperatura interior/exterior | Sensirion STS40 | Condiciones de conservación de fármacos termolábiles |

**Sensores médicos integrados:**

| Sensor | Modelo | Parámetros |
|---|---|---|
| Pulsioximetría | Masimo SET® ([masimo.com](https://www.masimo.com)) | SpO2, frecuencia cardíaca, índice de perfusión, MetHb, COHb |
| ECG | Philips Tempus ALS | 12 derivaciones, análisis STEMI automático, desfibrilador integrado |
| Presión arterial | SunTech Tango M2 | PANI no invasiva, medición continua en movimiento |
| Capnografía | Philips Capnostream 20p | EtCO2, frecuencia respiratoria, forma de onda (waveform) |
| Glucemia | Abbott FreeStyle Libre 3 | Monitorización continua intersticial, alarmas configurable |
...
| Motor Logístico | OSMnx + NetworkX + OpenStreetMap | Navegación, grafos de rutas, optimización A* con pesos dinámicos |
...
### 3.9 Integración V2I (Vehicle-to-Infrastructure)

El gemelo se conecta con el ecosistema urbano a través de APIs estandarizadas:

- **Control de tráfico** (estándar ETSI ITS / Open Traffic Standard): solicitud de ola verde para ambulancias, con reserva de tiempo en semáforos gestionados digitalmente.
- **Hospitales** (HL7 FHIR R4 — [hl7.org/fhir/R4](https://hl7.org/fhir/R4/)): notificación anticipada de llegada, reserva de cama UCI o quirófano, transmisión de constantes vitales del paciente en tránsito. FHIR R4 es el estándar de interoperabilidad hospitalaria más adoptado globalmente, con más de 71 % de países ya implementándolo ([Wikipedia FHIR](https://en.wikipedia.org/wiki/Fast_Healthcare_Interoperability_Resources)).
- **Meteorología** (AEMET OpenData API — [opendata.aemet.es](https://opendata.aemet.es)): alertas de nieve, hielo, lluvia intensa o niebla que afecten a la ruta en tiempo real.
- **SAMUR/SUMMA 112**: integración con el sistema de despacho mediante API REST para actualización de disponibilidad y estado de cada unidad.

### 3.10 Patrones de Diseño Implementados
...
### 3.11 Métricas de Escalabilidad del Sistema
...
### 3.12 Flujo de Datos Completo: De Sensor a Decisión
...
### 3.13 Integración con CAN Bus y Protocolo OBD-II
...
El sistema combina dos familias de algoritmos de detección de anomalías no supervisadas:

**Isolation Forest** (Liu et al., 2008): Aísla observaciones anómalas construyendo árboles de decisión aleatorios. Las anomalías requieren menos particiones para ser aisladas, lo que les da una puntuación de anomalía más alta. Es especialmente eficiente para datos de alta dimensionalidad y en tiempo real.

**Autoencoder LSTM** (Long Short-Term Memory): Red neuronal que aprende una representación comprimida (latente) de los patrones normales de telemetría. Las anomalías generan un error de reconstrucción elevado, que sirve como señal de alerta. La arquitectura LSTM es idónea para series temporales con dependencias temporales complejas.
...
|---|---|---|---|
| Motor (sistema de lubricación) | LSTM Bidireccional + Modelo Físico ODE | 72–120 h | Temperatura, vibración (acelerómetro), presión de aceite, horas de operación |
...
#### 4.4.1 Algoritmo Base: A* con Pesos Dinámicos

El sistema utiliza el algoritmo **A* (A-estrella)** aplicado sobre un grafo de la red viaria urbana obtenido de **OpenStreetMap** mediante la librería **OSMnx** ([osmnx.readthedocs.io](https://osmnx.readthedocs.io/)). A diferencia de los algoritmos de navegación convencionales, los pesos de los arcos del grafo se actualizan dinámicamente:

- **Factor de tráfico** (w_tráfico): actualizado cada 5 minutos desde la API de tráfico de la DGT o proveedores como TomTom/HERE.
- **Factor de prioridad de emergencia** (w_prioridad): los tramos con semáforos con protocolo V2I tienen un coste reducido.
- **Factor de confianza temporal** (w_confianza): la información de tráfico con más de 5 minutos de antigüedad pierde peso de forma lineal.
- **Factor meteorológico** (w_meteorología): ajuste de velocidades seguras en lluvia, nieve o niebla (datos AEMET).

Las pruebas de simulación con datos históricos de Madrid muestran una **reducción del 15-25 %** en tiempos de viaje respecto al ruteo estático convencional (dependiente de la densidad de tráfico y la integración V2I disponible).
...
**Vista 1: Mapa de Flota (vista principal)**

La vista principal muestra un mapa interactivo de la ciudad con todas las ambulancias activas representadas como iconos codificados por color y estado:

- 🟢 **Verde**: ambulancia disponible, en buenas condiciones mecánicas.
- 🟡 **Amarillo**: en misión no urgente o en traslado de vuelta.
- 🟠 **Naranja**: en misión urgente / en camino a emergencia.
- 🔴 **Rojo**: emergencia crítica en curso (PCR, trauma grave, STEMI).
- ⚫ **Gris**: fuera de servicio (mantenimiento, combustible, turno).
- 🔵 **Azul parpadeante**: alerta mecánica activa (requiere atención pronto).

Al hacer clic en una ambulancia, se despliega un panel lateral con:
- Constantes vitales del paciente en tiempo real (si hay paciente).
- Estado mecánico resumido (combustible, temperatura motor, alertas activas).
- Ruta actual con ETA al destino.
- Historial de misiones del turno.
- Botón de contacto (radio simulada sobre VOIP/5G).

**Vista 2: Panel de Alertas**

Lista priorizada de todas las alertas activas del sistema, ordenadas por severidad y tiempo de generación:

| Prioridad | Tipo | Ambulancia | Descripción | Tiempo |
|---|---|---|---|---|
| 🔴 CRÍTICA | Anomalía médica | AMB-MAD-003 | SpO2 paciente < 85 % durante > 60 s | Hace 23 s |
| 🟠 ALTA | Mecánica | AMB-MAD-007 | Temperatura motor > 105 °C — RUL motor: 8 h | Hace 2 min |
| 🟡 MEDIA | Combustible | AMB-MAD-012 | Combustible < 15 % — autonomía ~45 km | Hace 5 min |
| 🔵 INFO | Comunicación | AMB-MAD-015 | Latencia MQTT > umbral (280 ms) — usando backup | Hace 8 min |
...
**Vista 4: Simulación What-If (para planificación)**

Interfaz para ejecutar simulaciones prospectivas antes o durante el turno:
- Definir escenario (corte de 4G, hospital saturado, evento masivo).
- Visualizar impacto en tiempos de respuesta y disponibilidad de recursos.
- Obtener recomendaciones automáticas (repositionamiento preventivo de ambulancias).
...
La tecnología más avanzada falla si las personas que deben usarla no la adoptan. La gestión del cambio organizacional es, según la literatura de transformación digital en sanidad, el determinante más importante del éxito o fracaso de implantaciones como esta.

#### 6.8.1 Barreras Identificadas y Estrategias de Superación

| Barrera | Frecuencia en literatura | Estrategia del proyecto |
|---|---|---|
| **Resistencia al cambio** ("siempre lo hemos hecho así") | Muy alta | Co-diseño con usuarios desde el día 1; demostración temprana de valor |
| **Miedo a la vigilancia excesiva** (los datos del conductor monitorizados) | Alta | Acuerdo claro sobre uso de datos; datos mecánicos y de ruta sí; no monitoreo de comportamiento personal |
| **Desconfianza en la IA** ("la máquina me va a decir qué hacer") | Alta | IA como asistente, nunca como decisor; el profesional tiene siempre la última palabra |
| **Sobrecarga de alertas** (fatiga por alertas) | Alta | Umbral conservador inicial; ajuste progresivo con feedback del usuario; regla: < 5 alertas/turno/ambulancia |
| **Complejidad tecnológica percibida** | Media | UI diseñada para máxima simplicidad; formación por roles, no técnica |
| **Falta de tiempo para formación** | Media | Módulos cortos (15-30 min); gamificación; formación en el propio vehículo con simulador embarcado |

#### 6.8.2 Plan de Gestión del Cambio (Modelo Kotter 8 Pasos)

El plan sigue el **modelo de 8 pasos de Kotter** ([Kotter, *Leading Change*, 1996](https://www.kotterinc.com/methodology/8-steps/)), ampliamente validado en transformaciones organizacionales sanitarias:

| Paso | Actividad | Responsable | Plazo |
|---|---|---|---|
| 1. Crear sentido de urgencia | Presentación de datos reales: mortalidad evitable por tiempos de respuesta; benchmark King County | Dirección + Equipo técnico | Mes 1 |
| 2. Formar coalición guía | Grupo de trabajo con jefes de SAMUR/SUMMA + técnicos médicos referentes + IT | Gerencia SEM | Mes 1 |
| 3. Visión y estrategia | Definir la visión compartida: "Salvar más vidas con datos" | Coalición guía | Mes 2 |
| 4. Comunicar la visión | Sesiones informativas a toda la plantilla; vídeo explicativo del proyecto | RRHH + Comunicación | Mes 2 |
| 5. Eliminar obstáculos | Resolver dudas sobre privacidad; adaptar convenio colectivo si necesario | RRHH + Comité de empresa | Meses 2-3 |
| 6. Generar victorias rápidas | Demo del dashboard con datos reales del primer vehículo; mostrar primera alerta de mantenimiento predictiva correcta | Equipo técnico | Mes 3 |
| 7. Consolidar avances | Ampliar piloto; premiar early adopters; publicar resultados internos | Gerencia + Equipo técnico | Meses 4-9 |
| 8. Anclar en la cultura | Incluir el uso del gemelo en los procedimientos operativos normalizados (PON); métricas del sistema en evaluación del desempeño | Gerencia + RRHH | Mes 10+ |

#### 6.8.3 Indicadores de Adopción
...
1. **T−72h**: El modelo LSTM predice RUL del motor en 68 horas con confianza del 87 %. Se genera alerta amarilla y se programa revisión para el siguiente cambio de turno.
...
### 9.1 Hardware Detallado

**HPE Edgeline EL8000 — Especificación completa por ambulancia** ([HPE Store](https://buy.hpe.com/us/en/compute/edgeline-systems/c/1008663980)):

| Parámetro | Especificación |
|---|---|
| Procesador | Intel Xeon D-2146NT, 8 núcleos, 2,0 GHz base / 3,0 GHz boost |
| Memoria | 64 GB DDR4-2400 ECC RDIMM (ampliable a 512 GB) |
| Almacenamiento primario | 2× M.2 NVMe SSD 1 TB, RAID 1 hardware |
| Almacenamiento secundario (buffer) | 2× SATA SSD 960 GB para telemetría offline |
| Conectividad Ethernet | 4× 10GbE SFP+ (2 para MQTT, 1 para gestión, 1 para backup) |
| Conectividad inalámbrica | 1× módulo 5G (Qualcomm X55) + 1× WiFi 6E (802.11ax) |
| Conectividad satelital | 1× módulo Iridium/Starlink (fallback emergencia) |
| Seguridad hardware | Intel Boot Guard + TPM 2.0 + Intel TXT |
| Temperatura operativa | −20 °C a +60 °C (operación) / −40 °C a +70 °C (almacenamiento) |
| Humedad | 5–95 % sin condensación |
| Certificaciones | IP67, MIL-STD-810G (vibración, impacto, altitud, temperatura, humedad) |
| Dimensiones (U) | 1U short-depth, 42,5 × 43 × 5,0 cm |
| Peso | 7,2 kg |
| Alimentación | 12 VDC / 24 VDC (alimentación directa desde batería de vehículo) |
| Consumo máximo | 45 W |
| UPS integrada | Supercapacitor para 60 s de operación ante pérdida de alimentación |

### 9.2 Software y Stack Tecnológico
...
### 9.4 Especificaciones Criptográficas
...
### 9.7 Ciclos de Mantenimiento Predictivo

| Componente | Modelo de predicción | Horizonte típico | Indicadores de degradación |
|---|---|---|---|
| Motor (sistema de lubricación) | LSTM + modelo físico | 72–120 h antes del fallo | Temperatura aceite > umbral, cambios en viscosidad inferida |
| Sistema de frenos | Survival Analysis (Cox PH) | 100–500 km antes del límite | Presión hidráulica anómala, temperatura de disco, patrones de frenado |
| Batería de vehículo | Deep Q-Network | 7–14 días | Voltaje de carga, resistencia interna estimada, ciclos de descarga profunda |
| Neumáticos | Random Forest | 5.000–10.000 km | Presión, temperatura, patrones de vibración por frecuencia |
| Transmisión | Gradient Boosting | 500–2.000 h | Temperatura del aceite de caja, ruido vibratorio (análisis FFT) |
| Sistema eléctrico (alternador, fusibles) | SVM + regresión | 30–90 días | Voltaje de salida del alternador, consumo por circuito, temperatura de conectores |
| Suspensión y amortiguadores | Regresión lineal + umbral | 20.000–30.000 km | Acelerómetro vertical en baches, respuesta de frecuencia natural |

### 9.8 Diagrama de Secuencia: Protocolo Completo de Emergencia
...
- **Wikipedia** — Emergency medical services in Spain: [en.wikipedia.org/wiki/Emergency_medical_services_in_Spain](https://en.wikipedia.org/wiki/Emergency_medical_services_in_Spain)
...
- **Wikipedia FHIR** — Adopción global: [en.wikipedia.org/wiki/Fast_Healthcare_Interoperability_Resources](https://en.wikipedia.org/wiki/Fast_Healthcare_Interoperability_Resources)
...
- **OSMnx** — Network analysis with OpenStreetMap: [osmnx.readthedocs.io](https://osmnx.readthedocs.io/)
